...
import { Search, Plus, Minus, CreditCard, Receipt, Barcode, ShoppingBag, X } from 'lucide-react';
  ...
  return (
    <div className="h-[calc(100vh-140px)] flex flex-col lg:flex-row gap-6 animate-in fade-in duration-500">
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-4 gap-4 overflow-y-auto pr-1 pb-2">
          {filteredProducts.map(product => (
            <Card 
                ...
                className="cursor-pointer group hover:ring-2 hover:ring-zinc-900 transition-all border-zinc-200 overflow-hidden"
            >
              <div className="aspect-square bg-zinc-100 w-full relative">
                <img 
                    src={`https://picsum.photos/300/300?random=${product.id}`} 
                    alt={product.name} 
                    className="h-full w-full object-cover mix-blend-multiply opacity-80 group-hover:opacity-100 transition-opacity" 
                />
                <div className="absolute top-2 right-2 bg-white/90 backdrop-blur text-[10px] font-bold px-2 py-1 rounded-full text-zinc-900 shadow-sm">
                    {product.stock} left
                </div>
              </div>
              <div className="p-4">
                <h4 className="font-semibold text-zinc-900 truncate leading-none">{product.name}</h4>
                <p className="text-xs text-zinc-500 mt-1 mb-3">{product.sku}</p>
                <div className="flex justify-between items-center">
                  <span className="font-bold text-zinc-900">${product.price.toFixed(2)}</span>
                  <div className="h-6 w-6 rounded-full bg-zinc-900 text-white flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity">
      ...
      <div className="w-full lg:w-[400px] flex flex-col h-full">
        <Card className="flex flex-col h-full shadow-lg border-zinc-200 overflow-hidden">
            ...
            <CardContent className="flex-1 overflow-y-auto p-0">
                {cart.length === 0 ? (
                    ...
                    {cart.map(item => (
                        <div key={item.id} className="flex items-center p-4 hover:bg-zinc-50/50 transition-colors group">
                            <div className="flex-1 min-w-0 mr-4">
                                <h5 className="font-medium text-sm text-zinc-900 truncate">{item.name}</h5>
                                <p className="text-xs text-zinc-500">${item.price.toFixed(2)}</p>
                            </div>
                            
                            <div className="flex items-center gap-3 bg-zinc-50 rounded-md p-1 border border-zinc-100">
                                <button 
                                    onClick={() => updateQuantity(item.id, -1)} 
                                    className="h-6 w-6 flex items-center justify-center rounded hover:bg-white hover:shadow-sm text-zinc-600 transition-all"
                                >
                            ...
                            <div className="text-right w-20 ml-2">
                             ...
                             <button onClick={() => removeFromCart(item.id)} className="ml-2 p-2 text-zinc-400 hover:text-red-500 transition-colors">
                                <X className="h-4 w-4" />
                            </button>
                        </div>
                    ))}
            ...
            <div className="p-6 bg-zinc-50/80 border-t border-zinc-200 space-y-4 backdrop-blur-sm">
                <div className="space-y-2 text-sm">
                    ...
                    <div className="flex justify-between items-end pt-3 border-t border-zinc-200">
                        <span className="font-semibold text-base text-zinc-900">Total Due</span>
                        <span className="font-bold text-2xl text-zinc-900 tracking-tight">${total.toFixed(2)}</span>
                    </div>
                </div>
                <Button size="lg" className="w-full text-base shadow-zinc-900/10" disabled={cart.length === 0}>
